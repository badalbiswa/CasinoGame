...
Baccarat::Baccarat(int initialChips, int initialWinnings)
    : playerChips(initialChips), totalWinnings(initialWinnings), gameOver(false), currentBet(0), betChoice(0) {
    deck.shuffle();
}
...
void Baccarat::drawBankerThirdCard(const Card& playerThirdCard) {
    ...
    if (bankerSum <= 2) {
        ...
        bankerHand.push_back(deck.dealCard());
    } else if (bankerSum == 4 && (playerThirdCard.rank == Rank::TWO || playerThirdCard.rank == Rank::THREE ||
                                   playerThirdCard.rank == Rank::FOUR || playerThirdCard.rank == Rank::FIVE ||
                                   playerThirdCard.rank == Rank::SIX || playerThirdCard.rank == Rank::SEVEN)) {
        bankerHand.push_back(deck.dealCard());
    } else if (bankerSum == 5 && (playerThirdCard.rank == Rank::FOUR || playerThirdCard.rank == Rank::FIVE ||
                                   playerThirdCard.rank == Rank::SIX || playerThirdCard.rank == Rank::SEVEN)) {
        bankerHand.push_back(deck.dealCard());
    } else if (bankerSum == 6 && (playerThirdCard.rank == Rank::SIX || playerThirdCard.rank == Rank::SEVEN)) {
        bankerHand.push_back(deck.dealCard());
    }
}
...
int main() {
    ...
    do {
        ...
        std::cout << "Player's chips remaining: " << game.getPlayerChips() << std::endl;
